...
<div v-if="getItem.length != 0 && Activate.length !=0">
      <div class="row m-0">
        <div class="col-xl-12 breadcrumbs">
          <span class="h3 ml-2">Exports</span>
      ...
      <div class="row m-0" v-if="Activate[0].Export_Activate_Export_Notes === 'true'">
        <div class="col-xl-12 note_div">
          <p v-html="getItem[0].ExportNotes"></p>
        </div>
      </div>

      <!--  -->
      <div class="row m-0" v-if="Activate[0].Export_Activate_Export_Products === 'true'">
            ...
            >
              <template>
                <v-card class="mx-auto" max-width="344">
                  ...
                  <v-img :src="'http://landingsiteone.epizy.com/public/ftpimages/' + items.Image.img" height="200px"></v-img>
...
export default {
  ...
  methods: {
    initialize() {
      this.$http.get("/ExportNote").then(response => {
      ...
      this.$http.get("/Product").then(response => {
        ...
        for (let index = 0; index < response.data.IndexExport.length; index++) {
          const all_result = response.data.IndexExport[index];
      ...
      this.$http.get("/Product").then(response => {
        ...
        for (
          let index = 0;
          index < response.data.IndexAllExport.length;
          index++
        ) {
          const all_result = response.data.IndexAllExport[index];
          // //console.log.log(all_result);
